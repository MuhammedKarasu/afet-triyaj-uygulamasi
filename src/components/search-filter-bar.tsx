...
import { RISK, STATUS, type InterventionStatus } from "@/lib/constants";

export function SearchFilterBar({ query, risk, status, statusOptions = Object.keys(STATUS) as InterventionStatus[] }: { query: string; risk: string; status: string; statusOptions?: readonly InterventionStatus[] }) {
  return (
    <form className="panel mb-5 grid gap-3 p-3 sm:p-4 md:grid-cols-[1fr_180px_210px_auto]" method="get">
      <div className="relative"><Search className="absolute left-3.5 top-4 h-4 w-4 text-slate-400"/><input name="q" defaultValue={query} className="input pl-10" placeholder="Vaka kodu, ad, kimlik veya konum ara" aria-label="Yaralı ara"/></div>
